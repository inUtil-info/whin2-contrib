...
All whin nodes installed share a configuration node that stores your user credentials. Once you get subscribed an ApiKey will show up: copy it; then open the configuration node and paste it on the field named: _ApiKey_. That's all you need to do to start using whin.
...
The _ApiKey_ field is expecting a string, numbers and letters with no spaces. If you get stucked, check this [video](https://www.youtube.com/watch?v=uOZ-oH4kP58) and see where can you find the key that you have to copy and paste on the config node.
...
![whin-nodes](./icons/infographic-whin2.#flow/f6f2187d.f17ca8png)
...
This is the node we recommend you start using right after you complete the config-node set-up. Just select the configuration you saved:
...
This node allows you to configure the webhook URL where Whin will send incoming messages. You can:

- Set a default webhook URL in the node configuration
- Dynamically set the URL via input message
- Receive the API response as output
...
[
  {
    ...
    "type": "whin-webhook",
    "name": "Set Webhook",
    "url": "https://webhook.my-target-host.com/path"
  },
  {
    "id": "inject",
    "type": "inject",
...
#### Receiving Messages (Webhook Mode):

This is the ONLY mode available for receiving messages. To receive whatsapps in your Node-RED flows, you need to set up a webhook route:

1. Use the whin-webhook node to set your target URL
2. Configure a standard HTTP-IN node to receive POST requests
3. Set this route in the Whin API configuration

Key steps:

- Expose a route using tools like [ngrok](https://youtu.be/7FHbfo-wRtY), [cloudflare tunnels](https://youtu.be/mMyoH4-mOiA), or by opening a port
- The route will receive all whatsapps as HTTP POST requests
- Use the whin-webhook node to configure the target URL
...
You need to set the right payload schema so that the back-end understands the request you send, otherwise whin won't be able to route the message.
